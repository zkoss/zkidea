Feature: MVVM Binding Expression Property Navigation
  ...
  # --- ViewModel ID Resolution ---

  Scenario: Resolve ViewModel class from @id and @init declarations
    Given the ZUL file declares viewModel="@id('vm') @init('com.example.MyViewModel')"
    When I Ctrl+Click on "list" in the expression "@load(vm.list)"
    Then the plugin should resolve "vm" to "com.example.MyViewModel"
    And navigate to "MyViewModel.getList()" method

  Scenario: Resolve ViewModel with different @id name
    Given the ZUL file declares viewModel="@id('myVm') @init('com.example.MyViewModel')"
    And the expression is "@load(myVm.list)"
    When I Ctrl+Click on "list" in the expression "@load(myVm.list)"
    Then the plugin should resolve "myVm" to "com.example.MyViewModel"
    And navigate to "MyViewModel.getList()" method
