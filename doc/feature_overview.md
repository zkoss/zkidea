...
| `ErrorPageRenderer` | `main/java/.../ErrorPageRenderer.java` | Turns a `RenderError` into a self-contained, theme-aware, HTML-escaped error page shown in the preview pane when a `.zul` fails to render (L-10). The structured `RenderError`/`toJson()` contract is unchanged — this only replaces the browser-facing bytes. |
