...
| `ZulPreviewFileEditor` | The preview half of the split. Gated on `JcefAvailability.probe()`: if JCEF is unavailable it shows the diagnosis plus an "open in external browser" link instead of a browser. Otherwise asks `ZulPreviewServerService` to prepare a preview target, then points a `PreviewBrowser` at `http://localhost:<port><requestPath>`; if building it throws anyway, that becomes an `INITIALIZATION_FAILED` card rather than a pane stuck on "Starting ZK preview server…". **Names no JCEF type** — see `PreviewBrowser`/`JcefPreviewBrowser` and issue #66. A `MergingUpdateQueue`-debounced `BulkFileListener` reloads the browser on VFS content-change events (i.e. after save — unsaved in-editor changes do not refresh). An `onBeforeBrowse` handler keeps non-loopback http(s) navigation out of the pane and hands it to the external browser. All child resources (browser, listener, refresh queue) are parented to `this` via `Disposer.register`, so `Disposer.dispose(this)` (fired when the editor tab closes) tears them down automatically. On a successful render it pins the hint banner (see `LayoutPreviewHint`) above the browser. The browser is built via `JBCefBrowser.createBuilder()` with `setEnableOpenDevToolsMenuItem(true)`, so the pane's context menu offers **Open DevTools** — the only in-IDE way to see a JS error or a failed `/zkau/web/*` fetch behind a blank render. `showRenderedHtml` opens a `getSource()` dump (see `PreviewContextMenu`) as a read-only `LightVirtualFile` editor tab, closing the previous dump first; the EDT hop from CEF's visitor thread happens here, not in the handler. |
| `PreviewContextMenu` | `CefContextMenuHandlerAdapter` giving the pane a working "show me the source" entry. CEF's own **View Source** item is dead in an embedded browser — the command is handled natively inside CEF and routes into Chromium's open-in-a-new-tab path, and a `JBCefBrowser` in an editor split has no tab to open into, so it silently does nothing; there is no Java-side hook to repair it. So `customize()` removes `MENU_ID_VIEW_SOURCE` and adds **View Rendered HTML** at `MENU_ID_USER_FIRST` (**not** `MENU_ID_USER_LAST` — IntelliJ's own `DefaultCefContextMenuHandler`, which shares this browser's `JBCefClient`, claims that id for *Open DevTools*, so reusing it would silently shadow DevTools). The command calls `CefBrowser.getSource`, which yields the **live DOM markup** rather than the response bytes — the distinction that matters for ZK, whose response is a `zkmx([...])` bootstrap the client engine expands into DOM, so only the DOM answers "is my component rendered but hidden?". Every other command id returns `false` so CEF's own items and IntelliJ's DevTools item still fire. Menu surgery and command routing are pure and unit-tested; display is the caller's job. |
...
| `JcefAvailability` | Diagnosis of *why* the embedded browser is unavailable, each with the remedy shown on the card — `BOOT_JDK_NO_JCEF` (boot runtime isn't a JetBrains Runtime), `JCEF_PLUGIN_UNAVAILABLE` (2026.2+: the bundled **Web Browser (JCEF)** plugin is disabled or missing, so the classes aren't on our classloader — issue #66), `REGISTRY_DISABLED` (`ide.browser.jcef.enabled` off), `INITIALIZATION_FAILED` (JCEF is there but the browser wouldn't start; carries the failure), `INCOMPATIBLE` (catch-all). `diagnose(classesLoadable, registryEnabled, vendor)` is pure and unit-tested; `probe()` is the live call. `probe()` keys "classes missing" on `NoClassDefFoundError` specifically — `ExceptionInInitializerError` is a `LinkageError` too, and blaming the plugin for it would send the user to a setting that is already correct. |
